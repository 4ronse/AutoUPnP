...
public class Gateway {
    ...
    protected Map<String, String> command(String action, Map<String, String> params) throws Exception {
    ...
    public boolean openPort(int port, boolean udp) {
        return openPort(iface.getHostAddress(), port, port, "WaifUPnP", udp);
    }

    public boolean openPort(String ip, int internalPort, int externalPort, String desc, boolean udp) {
        if (internalPort < 0 || internalPort > 65535) throw new IllegalArgumentException("Invalid internalPort");
        if (externalPort < 0 || externalPort > 65535) throw new IllegalArgumentException("Invalid externalPort");

        Map<String, String> params = new HashMap<String, String>();
        params.put("NewRemoteHost", "");
        params.put("NewProtocol", udp ? "UDP" : "TCP");
        params.put("NewInternalClient", ip);
        params.put("NewExternalPort", "" + externalPort);
        params.put("NewInternalPort", "" + internalPort);
        params.put("NewEnabled", "1");
        params.put("NewPortMappingDescription", desc);
